use bevy::prelude::*;
use shared::{
    CurrentTick, event::{NetEntId, PlayerId, UDPacketEvent, server::CastSkillUpdate}, net_components::{ents::SendNetworkTranformUpdates, ours::ControlledBy}, netlib::{ServerNetworkingResources, send_outgoing_event_next_tick}, skills::animations::{SharedAnimationPlugin, UsingSkillSince}
};
...
fn on_unit_begin_skill_use(
    ...
    mut our_unit: Query<
        (&NetEntId, Entity, Option<&mut UsingSkillSince>, &ControlledBy),
    ...
    for packet in skill_change.read() {
        let Some(player_id) = endpont_to_player.map.get(&packet.endpoint) else {
            warn!("Received CastSkillUpdate from unknown endpoint: {:?}", packet.endpoint);
            continue;
        };
        ...
        for (ent_id, entity, maybe_existing_skill, controlled_by) in &mut our_unit {
            ...
            if !controlled_by.players.contains(&player_id) {
                warn!(?player_id, ?ent_id, "Player tried to cast skill on unit they do not control");
                cancelled = true;
                continue;
            }
            ...
            if let Some(mut existing_cast) = maybe_existing_skill {
                ...
                if existing_cast.skill == packet.event.skill {
                    if packet.event.begin_casting {
                        ...
                        info!(?player_id, ?ent_id, "Player tried to begin casting a skill they are already casting");
                        break;
                    }
                }
