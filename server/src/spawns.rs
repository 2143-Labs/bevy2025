...
fn on_circle_spawn(
    ...
    for spawn in spawns.read() {
        ...
        match &spawn.data.unit {
            ...
            shared::event::UnitType::Spawn { npc_type } => {
                base.insert((Spawn_type.clone(), npc_type.get_ai_component()));
            }
        };
...
fn apply_Spawn_movement_intents(
    mut Spawns: Query<(&mut Transform, &mut MovementIntention, &AttackIntention), (With<AIType>, Without<Controlled>)>,
    non_ai: Query<(&Transform, &MovementIntention), With<Controlled>>,
    time: Res<Time>,
) {
    // Apply all the movement
    for (mut ply_tfm, ply_intent, attack_intent) in &mut Spawns {
        let delta_target =
    ...
    for _i in 0..3 {
        ...
        let mut all_Spawn_positions = Vec::with_capacity(npcs.iter().len());
        for (ply_tfm, _, _) in &Spawns {
            all_Spawn_positions.push(ply_tfm.translation);
        }
        let all_player_positions: Vec<_> = non_ai.iter().map(|x| x.0.translation).collect();

        // Call this function to get an iterator
        let all_positions = || all_Spawn_positions.iter().chain(all_player_positions.iter());

        // Now, check for all other collisions
        // TODO: This is O(n^3). If len Spawns > 1000, maybe log a warning that we need to rewrite
        // this?
        for (mut ply_tfm, mut ply_intent, _) in &mut Spawns {
